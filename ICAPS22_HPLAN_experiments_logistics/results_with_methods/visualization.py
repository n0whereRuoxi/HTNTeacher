        ...
        for is_complete in complete:
            for is_curriculum in curriculum:
                for is_prune in prune:
                    with open("{}{}{}{}{}.txt".format(domain,'_complete' if is_complete else '', '_curriculum' if is_curriculum else 'original', '_prune' if is_prune else '', '_prec' if has_prec else ''), newline='') as csvfile:
                        ...
                        axs[0].errorbar(x, y_number_of_methods, yerr=e_number_of_methods, 
                            ...
                            label="{}{}{}{}".format('Teachable-HTN-Maker' if is_curriculum else 'HTN-Maker', ', pruning' if is_prune else '', ', prec' if has_prec else '', ', complete' if is_complete else ''),
                            ls='-' if is_prune else '--')
