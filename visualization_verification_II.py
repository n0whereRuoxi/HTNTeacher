    ...
    for mode in modes:
        for is_prune in prune:
            for has_precondition in prec:
                with open(dir + domain + "/results_with_methods/planmeta_{}_{}{}{}.txt".format(domain, mode, '_prune' if is_prune else '', '_prec' if has_precondition else ''), newline='') as csvfile:
